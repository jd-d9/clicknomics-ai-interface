                ...
                <v-col cols="12" sm="12" md="12" lg="12" class="py-0">
                    <v-breadcrumbs>
                        <div class="d-flex">
                            ...
                            <span>Google Accounts Payment Declined</span>
                        </div>
                    </v-breadcrumbs>
                </v-col>
                ...
                <v-col cols="12" sm="12" md="12" lg="12" class="py-0" v-if="permissions.view == '1' && !showLoader">
                    <v-card class="card_design mb-4">
                        <v-card-title class="d-flex align-center justify-end">
                            Google Accounts Payment Declined List
                            <v-spacer></v-spacer>
                        ...
                        <v-data-table class="table-hover-class mt-4" :footer-props="{'items-per-page-options': [5, 10, 15, 25, 50, 100, -1]}" :headers="headers" :items="urlList" :search="search" :itemsPerPage="itemsPerPage">
                            ...
                            <template v-slot:[`item.billing_id`]="{ item }">
                                {{item.selectable.billing_id ? item.selectable.billing_id : '-'}}
                            </template>
                            <template v-slot:[`item.billing_setup_status`]="{ item }">
                                {{item.selectable.billing_setup_status ? item.selectable.billing_setup_status : '-'}}
                            </template>
                        </v-data-table>
                    </v-card>
                </v-col>
...
export default {
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Billing Setup Status', key: 'billing_setup_status' },
            ],
            urlList: [],
            singleExpand: true,
            page: 1,
    ...
    methods: {
        ...
        getData() {
            ...
            axios.get(this.$api + '/monitoring/paymentDeclined/google', {
                headers: {
