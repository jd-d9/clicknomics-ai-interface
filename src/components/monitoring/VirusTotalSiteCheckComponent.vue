    ...
    <div class="bg-default main-content-height">
                ...
                <v-col cols="12" sm="12" md="12" lg="12" class="py-0">
                    <v-breadcrumbs>
                        <div class="d-flex">
                            ...
                            <span>Virus Total Malware Site Check</span>
                        </div>
                        <v-spacer/>
                ...
                <v-col cols="12" sm="12" md="12" lg="12" class="py-0" v-if="permissions.view == '1' && !showLoader">
                    <v-card class="card_design mb-4">
                        <v-card-title class="d-flex align-center justify-end">
                            Virus Total Malware Site Check List
                            <v-spacer></v-spacer>
                        ...
                        <v-data-table class="table-hover-class mt-3" :headers="headers" :items="urlList" :items-per-page="itemsPerPage" show-select v-model="selected" :search="search">
                            ...
                            <template v-slot:[`item.error_message`]="{ item }">
                                <td v-if="item.selectable.last_analysis_stats">
                                    <i class="fas fa-exclamation-triangle mr-2" style="color:red" v-if="errorCount(item.selectable) > 0"></i>
                                    <i class="fas fa-check-circle mr-2" style="color:green" v-if="errorCount(item.selectable) === 0"></i>
                                    {{ item.selectable.error_message }}
                                </td>
                                <td v-else>
                                    -
                                </td>
                            </template>
                            <template v-slot:[`item.last_analysis_results`]="{ item }">
                                <td v-if="item.selectable.last_analysis_stats && errorCount(item.selectable) > 0">
                                    <router-link to="" @click="preview(item.selectable)">
                                        <img src="/assets/img/icons/eye.svg" style="width:25px">
                                    </router-link>
                                </td>
                                <td v-else>
                                    -
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Import Virus Total Malware Site Check</h5>
        ...
        <div class="modal fade" id="adddomain" tabindex="-1" role="dialog" aria-labelledby="adddomainTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Add Virus Total Malware Site Check</h5>
        ...
        <div class="modal fade" id="previewModal" tabindex="-1" role="dialog" aria-labelledby="createUpdateDataTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 style="color:#fff;" class="modal-title">Flagged Security Vendors Details</h5>
                        <button type="button" class="close" aria-label="Close" @click.prevent="closePreviewModal">
                        ...
                        <div class="col-12 message-container">
                            <ol class="pl-4">
                                <li v-for="(item , index) in resultData" :key="index">
                                    <i class="fas fa-exclamation-triangle mr-2" style="color:red"></i>
                                    <span><b>{{item.name}}</b></span>
                                    <span class="ml-7 float-right text-capitalize"><b>{{item.value.result}}</b></span>
                                    <hr class="my-3">
                                </li>
                            </ol>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Detection Result', key: 'error_message' },
                { title: 'Flagged Security Vendors Details', key: 'last_analysis_results', sortable: false,},
    ...
    methods: {
        ...
        getData() {
            ...
            axios.get(this.$api + '/monitoring/virustotal', {
                headers: {
        ...
        // error count
        errorCount(item) {
            const stats = JSON.parse(item.last_analysis_stats)
            const total = parseInt(stats.malicious) + parseInt(stats.suspicious)
            return  total;
        },
        // preview detail
        preview(list) {
            const data  = JSON.parse(list.last_analysis_results);
            ...
            this.resultData = dataArray.filter(item => {
                return item.value.result != 'clean' && item.value.result != 'unrated';
            });
        ...
        addNew() {
            this.showLoader = true;
            axios.post(this.$api + '/monitoring/virustotal', {
        ...
        deleteData(id) {
            this.showLoader = true;
            axios.delete(this.$api + '/monitoring/virustotal/' + id, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: this.getAccessToken()
                }
        ...
        deleteSelected() {
            ...
            if(confirmDelete) {
                ...
                axios.post(this.$api + '/monitoring/deleteVirustotalMutipleRows', formData, {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: this.getAccessToken()
                    }
        ...
        importCsv() {
            this.showLoader = true;
            axios.post(this.$api + '/monitoring/importvirustotalCSV', {
